...
const ez = (i) => { const d = Math.min(1, i * k.D.rate / 60); // 1.23: más fácil (antes i/40) · k.D.rate: la rampa tarda más en fácil
  return d * d * (3 - 2 * d); };
...
k.run((dt) => {
  ...
  if (p.on) { const q = p.on; p.a += dt * (96 + 60 * ez(best)) * k.D.spd / q.r; walkT += dt; p.x = q.x + Math.cos(p.a) * (q.r + 8); p.y = q.y + Math.sin(p.a) * (q.r + 8);
    ...
    if (p.y < -400 || p.y > 760 || p.x < cam - 300 || p.air > 9 * k.D.time) return k.lose(CFG.id, score, 'Perdido en el espacio', `${best} planetas`); }
