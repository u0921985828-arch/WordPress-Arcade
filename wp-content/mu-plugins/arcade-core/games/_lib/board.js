...
/* Dificultad: k.D.cpu se suma al nivel de la IA al leerlo (lo guardado no cambia). */
const aiL = (max) => k.clamp((lvl | 0) + k.D.cpu, 0, max);
function rAI() { const ms = rMoves(b, 2); if (!ms.length) return null; const L = aiL(2);
...
function cAI() { const ss = cSeqs(b, 2); if (!ss.length) return null; const L = aiL(3); if (Math.random() < CERR[L]) return k.pick(ss);
...
function draw() {
  ...
  // HUD
  label(CFG.title, 16, 12, 20, '#ffe27a'); label(`Victorias ${wins}`, W - 16, 14, 15, '#fff', 'right'); label(`IA nivel ${aiL(M === 'reversi' ? 2 : 3) + 1}`, W - 16, 36, 12, '#cfd6ff', 'right');
...
function f4Main() {
  ...
  k.run((dt) => {
    ...
    if (over4) return;
    const p = turn;
    if (!human(p)) { think += dt; if (cpuPick < 0 && think > 0.25) { cpuPick = f4AI(g, p + 1, k.clamp(lvl4 + k.D.cpu, 0, 2)); } if (cpuPick >= 0) { if (hover[p] !== cpuPick && think > 0.4) { hover[p] += Math.sign(cpuPick - hover[p]); think = 0.25; k.sfx('click'); } else if (hover[p] === cpuPick && think > 0.55) drop(cpuPick); } return; }
    ...
    for (const p of [0, 1]) { const x = p ? W - 66 : 66, act = k.st === 'play' && !over4 && turn === p, col = k.pcol(p);
      ...
      label(seats && seats[p].cpu ? LVLN[k.clamp(lvl4 + k.D.cpu, 0, 2) | 0] : (act ? 'Tu turno' : ' '), x, 298, 15, act ? '#fff' : '#cfd6ff', 'center'); }
