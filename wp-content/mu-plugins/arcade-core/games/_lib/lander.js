...
function build() {
  ...
  const a = k.ri(3, 13) * 20 + 40; let b; do b = k.ri(2, 28) * 20; while (Math.abs(b + 20 - (a + 40)) < 110 || b < 20 || b + 40 > 620);
  pads = [{ x: a, w: 80, m: 1 }, { x: b, w: 40, m: 3 }];
  ground = []; let y = 300;
  for (let x = 0; x <= 640; x += 20) { const p = pads.find((q) => x > q.x && x <= q.x + q.w); if (!p) y = k.clamp(y + k.rnd(-28, 28) + (y > 320 ? -10 : 0), 175, 340); ground.push([x, y]); }
  for (const p of pads) { const py = ground[p.x / 20][1]; for (let i = p.x / 20; i <= (p.x + p.w) / 20; i++) ground[i][1] = py; p.y = py; }
  s = { x: 200, y: 40, vx: 40, vy: 0, a: 0, thr: false }; maxFuel = fuel = Math.max(40, 100 - level * 8); landed = 0; crash = 0; smoke = []; debris = [];
  terrCv = off(640, 360, (g) => {
    g.beginPath(); g.moveTo(0, 360); ground.forEach(([x, yy]) => g.lineTo(x, yy)); g.lineTo(640, 360); g.closePath();
    const gr = g.createLinearGradient(0, 170, 0, 360); gr.addColorStop(0, '#8d8fa8'); gr.addColorStop(1, '#3b3d56'); g.fillStyle = gr; g.fill();
    g.save(); g.clip();
    for (let i = 0; i < 22; i++) { const x = rnd(i + level * 31) * 640, yy = gy(x) + 14 + rnd(i * 7 + level) * 120, r = 5 + rnd(i * 3.3) * 14; g.fillStyle = 'rgba(40,40,70,.35)'; g.beginPath(); g.ellipse(x, yy, r, r * 0.45, 0, 0, R2); g.fill(); g.strokeStyle = 'rgba(255,255,255,.18)'; g.lineWidth = 1.5; g.beginPath(); g.ellipse(x, yy + 1, r, r * 0.45, 0, 0.2, Math.PI - 0.2); g.stroke(); }
    for (let i = 0; i < 60; i++) { const x = rnd(i * 5 + level) * 640; g.fillStyle = 'rgba(255,255,255,.12)'; g.fillRect(x, gy(x) + 6 + rnd(i) * 150, 2, 2); }
    g.restore();
...
const SKY = off(640, 360, (g) => {
  const gr = g.createLinearGradient(0, 0, 0, 360); gr.addColorStop(0, '#04050f'); gr.addColorStop(1, '#161a3c'); g.fillStyle = gr; g.fillRect(0, 0, 640, 360);
  for (let i = 0; i < 140; i++) { g.fillStyle = `rgba(255,255,255,${0.25 + rnd(i) * 0.7})`; const r = rnd(i + 9) < 0.1 ? 1.4 : 0.8; g.beginPath(); g.arc(rnd(i + 1) * 640, rnd(i + 2) * 300, r, 0, R2); g.fill(); }
  // Tierra al fondo
  const ex = 420, ey = 92, er = 30; g.fillStyle = 'rgba(92,170,255,.15)'; g.beginPath(); g.arc(ex, ey, er + 8, 0, R2); g.fill();
  ...
  // colinas lejanas
  g.fillStyle = '#23264a'; g.beginPath(); g.moveTo(0, 360); for (let x = 0; x <= 640; x += 16) g.lineTo(x, 210 - Math.sin(x / 90) * 26 - Math.sin(x / 37) * 8 - rnd(Math.floor(x / 16)) * 6); g.lineTo(640, 360); g.fill();
});
reset(); k.show(CFG.title, '← → girar · ↑ o mantener pulsado = motor (el dedo también orienta la nave). Aterriza suave y recto en una plataforma: la estrecha vale el triple.');
...
function footY() { return Math.min(gy((s.x + 640) % 640), gy((s.x - 12 + 640) % 640), gy((s.x + 12) % 640)); }

k.run((dt) => {
  tm += dt;
  for (const p of smoke) { p.x += p.vx * dt; p.y += p.vy * dt; p.vx *= 1 - dt * 2; p.vy *= 1 - dt * 2; p.t -= dt; p.r += dt * 10; } smoke = smoke.filter((p) => p.t > 0);
  for (const d of debris) { d.x += d.vx * dt; d.y += d.vy * dt; d.vy += 30 * dt; d.a += d.va * dt; if (d.y > gy(k.clamp(d.x, 0, 640))) { d.y = gy(k.clamp(d.x, 0, 640)); d.vy *= -0.3; d.vx *= 0.6; d.va *= 0.5; } }
  if (!k.gate(reset)) return;
  if (crash) { crash -= dt; if (crash <= 0) return k.lose('lunar-lander', score, 'Estrellado', `Nivel ${level}`); return; }
  ...
  c.drawImage(SKY, 0, 0, 640, 360);
  for (let i = 0; i < 6; i++) { c.globalAlpha = Math.max(0, Math.sin(tm * 2 + i * 1.7)); c.fillStyle = '#fff'; const x = rnd(i + 500) * 640, y = rnd(i + 600) * 150; c.fillRect(x - 3, y - 0.5, 6, 1); c.fillRect(x - 0.5, y - 3, 1, 6); } c.globalAlpha = 1;
  c.drawImage(terrCv, 0, 0, 640, 360);
  ...
  label('Combustible', 140, 53, 11, '#b8b6e0');
  const bv = s.vy > 40, bh = Math.abs(s.vx) > 25, ba = Math.abs(s.a) > 0.3;
  const row = (txt, v, bad, y) => { label(txt, 560, y, 11, '#b8b6e0', 'right'); label(v, 628, y - 2, 14, bad ? '#ff6b6b' : '#7cf7a0', 'right'); };
  ...
  if (landed) { const p = 2 - landed, sc = p < 0.2 ? 0.5 + p * 3 : 1.1; c.save(); c.translate(320, 130); c.scale(sc, sc); label('¡Aterrizaje!', 0, 0, 34, '#fff27a', 'center', 'middle'); label(`+${lastPts}`, 0, 34, 20, '#fff', 'center', 'middle'); c.restore(); }
  if (k.st === 'play' && fuel <= 0 && !landed && !crash) label('Sin combustible', 320, 150, 18, '#ff6b6b', 'center');
});
