...
<body><script>window.CFG={cfg};</script><script src="../_lib/kit.js?v=19"></script>{deps}<script src="../_lib/{eng}.js?v={ev}"></script></body></html>
'''
...
def main():
    ...
    for slug, (eng, cfg) in G.items():
        ...
        (d / 'index.html').write_text(TPL.format(title=titles[slug], cfg=json.dumps(cfg, ensure_ascii=False), eng=eng, ev=hashlib.md5((ENG_DIR / f'{eng}.js').read_bytes()).hexdigest()[:8], deps=''.join(f'<script src="../_lib/{d}.js?v=12"></script>' for d in deps_of(eng))), encoding='utf-8')
    ...
    for slug, (eng, cfg) in G.items():
        ...
        dep[slug] = ['_lib/kit.js?v=19'] + [f'_lib/{d}.js?v=12' for d in deps_of(eng)] + [f'_lib/{eng}.js?v={ev}']
