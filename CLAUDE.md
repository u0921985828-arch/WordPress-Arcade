...
## Qué es
Portal de juegos mobile-first en WordPress con **150 juegos propios** (100 + 50 de la oleada 1 de PLAN-400) (canvas 2D, sin librerías) servidos dentro de un plugin, más un importador opcional de catálogos profesionales (GamePix / GameDistribution). Objetivo: portal diferenciado y de calidad, monetizado con **AdSense**. Escalar a 900+ juegos.
...
- Versión actual del plugin: **1.24.0** (`const VERSION` en `wp-content/mu-plugins/arcade-core.php`).
...
- Caché de motores: `?v=` = hash md5 del motor (automático en build_games.py); `kit.js` (v=14) y `art.js` (v=9) siguen con `?v=N` manual.
