...
function bankPlay() { let d = [k.ri(1, 6), k.ri(1, 6), k.ri(1, 6), k.ri(1, 6), k.ri(1, 6)]; const nr = k.clamp((hands < 6 || Math.random() < 0.25 ? 1 : 2) + k.D.cpu, 0, 3); /* dificultad: k.D.cpu al nivel de la banca */ /* 1.23: más fácil: la banca repite solo 1 vez en las 6 primeras manos (y a veces después) */ for (let r = 0; r < nr; r++) { const cnt = [0, 0, 0, 0, 0, 0, 0]; d.forEach((v) => cnt[v]++); const best = cnt.indexOf(Math.max(...cnt)); if (rank(d)[0] >= 4) break; d = d.map((v) => (v === best && cnt[best] > 1 ? v : k.ri(1, 6))); } return d; }
