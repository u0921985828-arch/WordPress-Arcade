...
function build() {
  ...
  if (M === 'pipes') { N = Math.min(8, (k.dif === 2 ? 5 : 4) + Math.floor(level / (k.dif === 0 ? 4 : 3))); /* nivel 1-2: 4×4 */ S = Math.floor(432 / N); g = Array.from({ length: N * N }, () => ({ c: [0, 0, 0, 0], r: 0, ra: 0 })); src = [Math.floor(N / 2), Math.floor(N / 2)];
