...
function build() { N = Math.min(15, 7 + 2 * Math.floor(level / 2)); S = Math.floor(440 / N); OX = (480 - N * S) / 2; OY = 128; g = carve(N); goal = [N - 2, N - 2];
  ...
  const want = Math.min(cells.length / 5, 1 + Math.floor(level * 0.8 * k.D.rate)); // 1.23: más fácil (antes 1+nivel) · k.D.rate: menos agujeros en fácil
...
const LIVES0 = 4 + k.D.life;   // canicas: una más en fácil
function reset() { if (!level || (k.st === 'over' && lives <= 0)) { level = 1; score = 0; lives = LIVES0; } build(); }
/* ---------- Cachés ---------- */
...
k.run((dt) => {
  ...
  for (let s = 0; s < steps; s++) { const h = dt / steps; const A = (600 + 170 * Math.min(1, (level - 1) / 10)) * k.D.spd; ball.vx = (ball.vx + ax * A * h) * (1 - 0.8 * h); ball.vy = (ball.vy + ay * A * h) * (1 - 0.8 * h); ball.x += ball.vx * h; ball.y += ball.vy * h; collide(); }
  ...
  for (let i = 0; i < LIVES0; i++) { const x = 26 + i * 24, y = 62; c.globalAlpha = i < lives ? 1 : 0.25; c.drawImage(ballImg, x - 9, y - 9, 18, 18); } c.globalAlpha = 1;
