...
const L = LAND
  ? { top: 10, card: [24, 58, 752, 128], opt: (i) => [24 + (i % 2) * 384, 198 + (i >> 1) * 86, 368, 72], tfo: (i) => [24 + i * 384, 198, 368, 158], lec: (i) => [24 + i * 190, 370, 182, 76], fs: 25, of: 21 }
...
function draw() {
  ...
  const topY = L.top, pw = LAND ? 180 : 150;
  panel(cx0, topY, pw, 36, 12, '#3a3478', { drop: 3 });
  outlined(`${TF ? 'Afirmación' : 'Pregunta'} ${qi + 1}/${NQ}`, cx0 + pw / 2, topY + 18, 17, '#fff', 'center', 4);
  ...
  if (phase === 'ask' && qi < 2) {
    ...
    c.fillText(tx, W / 2, LAND ? 364 : 640);
  }
  seat.forEach((s, i) => lectern(i, s, t));
}
