...
function drumsGame() {
  ...
  function cpuPlan(q, n) {
    const pMiss = Math.max(0.03, 0.14 - cpuLv * 0.011), pPerf = Math.min(0.8, 0.4 + cpuLv * 0.04), r = Math.random();
  ...
  function finish() {
    ...
    if (hu.length === 1 && hu[0].score === top) lsSet('cpu:' + ID, Math.min(10, cpuLv + 1));
